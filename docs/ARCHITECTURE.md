# Архитектура проекта Aura

Краткое описание для новичка: **Aura** — это протокол займов под залог (lending). Пользователь вносит коллатерал (например wstETH), получает право занимать стейблкоин (USDC). Цена коллатерала берётся из оракула; протокол можно обновлять через прокси.
...
```
                    ┌─────────────────┐
                    │   AuraProxy     │  ← Адрес, который знает фронт и бэкенд
                    │ (прокси UUPS)   │     Все вызовы идут сюда.
                    └────────┬────────┘
                             │ delegatecall
                             ▼
                    ┌─────────────────┐
                    │  AuraEngine     │  ← Реальная логика: депозит, займ, repay,
...
│ marketId →       │ │ Chainlink +      │ │ shares ↔ assets │
│ vault, oracle,   │ │ fallback         │ │                 │
│ LTV, caps       │ └─────────────────┘ └─────────────────┘
└─────────────────┘
         │
         │  Один общий долг:
         ▼
┌─────────────────┐
│  USDC (debt)    │  ← Долг всегда в одной валюте (напр. USDC)
└─────────────────┘
```
...
- **AuraProxy**  
  Единственный контракт, с которым общается пользователь и фронт. Хранит адрес реализации (AuraEngine) и перенаправляет все вызовы туда через `delegatecall`. Позволяет обновлять логику (апгрейд) без смены адреса.

- **AuraEngine**  
  «Движок» протокола: депозит/вывод коллатерала по `marketId`, займ и погашение USDC, учёт долга и коллатерала, ликвидации, harvest (начисление дохода с коллатерала на долг), пауза и emergency shutdown. Хранит состояние в `AuraStorage` (EIP-7201), работает с реестром рынков и оракулом.

- **AuraMarketRegistry**  
  Реестр «рынков»: у каждого рынка есть vault (ERC-4626), оракул и параметры риска (LTV, порог ликвидации, штраф, кэпы, изоляция). Движок читает конфиг рынка из реестра и проверяет лимиты.
...
6. AuraProxy перенаправляет вызов в AuraEngine. Движок проверяет LTV, реестр рынков и оракул, списывает USDC с баланса движка и переводит пользователю.
